...
var collectionSetDescriptionCmd = &cobra.Command{
	Use:   "set-description @workspace/+name [--content STR | --file PATH]",
	Short: "Replace the description of a collection",
	Long: `Sets the short description on a collection. Provide exactly one of:
...
backend enforces a length limit, which surfaces as an error.`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		c, err := client.UpdateCollectionDescription(cmd.Context(), ref.Workspace, ref.Name, content)
		if err != nil {
		...
		fmt.Fprintf(cmd.OutOrStdout(), "Updated description on @%s/+%s\n", ref.Workspace, ref.Name)
		return nil
	},
}

func init() {
	collectionSetDescriptionCmd.Flags().String("content", "", "Description as an inline string")
	collectionSetDescriptionCmd.Flags().String("file", "", "Path to a file; use - to read from stdin")
}
